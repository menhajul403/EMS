                        ...
                        <li key={user.id} className="flex items-center justify-between rounded border p-3">
                            <span>
                                ...
                                <small className="text-gray-500">
                                    {user.email} · {user.roles?.map((role: any) => role.name).join(', ')}
                                </small>
                            </span>
                            <button type="button" onClick={() => router.delete(route('university.users.destroy', user.id))} className="text-red-600">
                                Delete
                            </button>
                        </li>
                    ))}
